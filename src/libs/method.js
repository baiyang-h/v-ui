 ...
 * @param { Boolean }  hump 是否时驼峰
 * @returns {string|boolean} 返回一个新的名字
 * @constructor
 */
export function ProduceComponentName(prefix=BRAND, name='', hump=true) {
  if(typeof prefix !== 'string' || typeof name !== 'string') return false
  if(hump) { // 驼峰写法
    const _prefix = prefix.slice(0,1).toUpperCase() + prefix.slice(1).toLowerCase()
    const _name = name.slice(0,1).toUpperCase() + name.slice(1).toLowerCase()
